...
nw_temp = tapply(nw_data[,2], nw_data[,1], paste) #apply系列都是用矩陣運算
...
image(nw)
windows()image(nw) # result - Figure 1 Level plot 圖跑不出來用這個
summary(nw)
...
rules1 = apriori(nw, parameter = list(support = 0.05, target = "maximally frequent itemsets")) # set parameters
